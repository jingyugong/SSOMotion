...
class GeometryTransformer():
    ...
    @staticmethod
    def convert_6D_to_9D_rot(x_batch):
        xt = x_batch[:,:3]
        xr = x_batch[:,3:9]
        xb = x_batch[:,9:]

        xr_mat = ContinousRotReprDecoder.decode(xr).view(-1, 9) # return [:,9]

        return torch.cat([xt, xr_mat, xb], dim=-1)
...
class BodyParamParser():
    ...
    def body_params_encapsulate_batch_hand(x_body_rec, body_pose_dim=32):
        assert x_body_rec.shape[1] == 40 + body_pose_dim
        body_params_batch_rec={}
        body_params_batch_rec['transl'] = x_body_rec[:,:3]
        body_params_batch_rec['global_orient'] = x_body_rec[:,3:6]
        body_params_batch_rec['betas'] = x_body_rec[:,6:16]
        body_params_batch_rec['body_pose'] = x_body_rec[:,16:16+body_pose_dim]
        body_params_batch_rec['left_hand_pose'] = x_body_rec[:,16+body_pose_dim:28+body_pose_dim]
        body_params_batch_rec['right_hand_pose'] = x_body_rec[:,28+body_pose_dim:]
